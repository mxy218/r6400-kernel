...
static struct ppp_channel backup_channel;
...
#define PPP_SHORTCUT
         ...
         */
        if (pch && pch->chan)
            memcpy(&backup_channel,pch->chan,sizeof(struct ppp_channel));
        else
        {
            pch->chan=&backup_channel;
            if(print_count<1000)
            {
                print_count++; 
printk(KERN_EMERG"bug!!!!! pppoe_crash\n\n");
            }
        }
